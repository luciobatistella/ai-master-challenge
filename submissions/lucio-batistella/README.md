...
- **Tratamento dos "sem conta":** em vez de escondê-los ou inventar um estágio de pipeline falso, eles aparecem no estágio real deles (Prospecting/Engaging) — no Kanban como uma 3ª coluna dedicada, na Lista ao final (sem posição no ranking, já que não é possível agir sobre um deal sem saber quem é o cliente) e isoláveis a qualquer momento via um filtro dedicado "Sem conta" no Estágio.
- **App funcional real:** lista priorizada + Kanban por estágio + Dashboard com gráficos (Chart.js) — todos os 3 sobre os mesmos filtros (vendedor/manager/região/estágio) com padrão rascunho→aplicar, busca, dark/light mode com paleta validada por contraste (WCAG), tudo responsivo mobile→desktop (Kanban é desktop-only, Lista e Dashboard funcionam nos dois), navegação de login/OTP fake (challenge não pediu autenticação real).
- **Dashboard:** medidor do impacto de "sem conta", pipeline por estágio, distribuição de prioridade (0–100), ranking de managers por valor esperado e distribuição por região — todos recalculados ao vivo conforme os filtros ativos, com paleta categórica validada (contraste + segurança daltonismo) pelo script `validate_palette.js`.
